        ...
        <div class="mx-auto max-w-4xl space-y-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h1 class="text-2xl font-semibold">Modifier {{ $project->name }}</h1>
                </div>
                <form action="{{ route('admin.onas-projects.destroy', $project) }}" method="post" onsubmit="return confirm('Supprimer ce projet ?');">
                    @csrf
                    @method('delete')
                    <button type="submit" class="rounded-full border border-red-400 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-red-300 hover:bg-red-500/10">
                        Supprimer
                    </button>
                </form>
            </div>
            ...
            <form action="{{ route('admin.onas-projects.update', $project) }}" method="post" class="space-y-6" enctype="multipart/form-data">
                @csrf
                @method('put')
